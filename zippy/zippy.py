        ...
        with open(fastafile, "w") as outfh:
            with open(inputfile) as infh:
                for i, line in enumerate(infh):
                    if i == 0:
                        minimalHeader = set(
                            ["primername", "primerset", "tag", "sequence", "vessel", "well"]
                        )
                        header = [x.lower() for x in line.rstrip().split("\t")]
                        try:
        ...
        for setname in setnames:
            try:
                ...
                try:
                    ...
                    print(
                        ">>", primersets[p.name], "|", p.name, "|", setnames, "<", file=sys.stderr
                    )
                    raise
            except:
                raise
            ...
            reverse = (
                p.targetposition.reverse if p.targetposition else parsePrimerName(p.name)[1] < 0
            )
            try:
    ...
    if primer3:
        ...
        for p in pairs.values():
            if not p[0].targetposition or not p[1].targetposition:
                ...
                if amplicons:
                    shortest = sorted(amplicons, key=lambda x: len(x[2]))[
                        0
                    ]  # sort amplicons by size
                    if len(amplicons) > 1:
                        print(
                            "WARNING: multiple amplicons for {}. Assuming shortest ({}bp) is correct.".format(
                    ...
                    for mapped, query in [[0, 1], [1, 0]]:
                        for l in p[mapped].loci:
                            newLoci[query] += refGenome.primerMatch(
                                l, p[query].seq, config["import"]["ampliconsize"]
                            )
                    # add new loci
                    if not newLoci[0] and not newLoci[1]:
                        print(
                            "WARNING: {} is not specific and not imported ({},{})".format(
                    ...
                    if amplicons:
                        ...
                        print(
                            "WARNING: Primer set {} has no valid amplicons ({},{})".format(
                                p.name, len(p[0].loci), len(p[1].loci)
                            ),
                            file=sys.stderr,
                        )
                    ...
                    print(
                        "WARNING: Primer set {} does not produce a well-sized, unique amplicon ({},{})".format(
                            p.name, len(p[0].loci), len(p[1].loci)
                        ),
                        file=sys.stderr,
                    )
...
def getPrimers(
    ...
    try:
        ...
        print(
            "Could not read blacklist cache from",
            config["blacklistcache"],
            "error:",
            exc,
            file=sys.stderr,
        )
    seqhash = lambda x, y: hashlib.sha1(
        ",".join([x, y])
    ).hexdigest()  # sequence pair hashing function
    ...
    if compatible:
        ...
        for tier in tiers:
            ...
            # get sequence
            maxFlank = max(
                [max(x) for x in config["design"]["primer3"][tier]["PRIMER_PRODUCT_SIZE_RANGE"]]
            )
            ...
            cfg["PRIMER_PRODUCT_SIZE_RANGE"] = [
                [x[0] + len(gap), x[1] + len(gap)] for x in cfg["PRIMER_PRODUCT_SIZE_RANGE"]
            ]
    ...
    if db:
        ...
        primers_found_in_DB_string = (
            "Found primers for {:d} out of {:d} intervals in database".format(
                len(intervals_in_database), len(intervals)
            )
        )
    ...
    if design:
        ...
        for tier in tiers:
            ...
            insufficentAmpliconIntervals = [
                iv for iv in intervals if config["report"]["pairs"] > len(ivpairs[iv])
            ]
            # print("leninsuf", len(insufficentAmpliconIntervals))
            if not insufficentAmpliconIntervals:
                break  # end design process
            print(
                "Round #{} ({} intervals)".format(tier + 1, len(insufficentAmpliconIntervals)),
                file=sys.stderr,
            )
            # Primer3 design
            designedPairs = {}
            ...
            for i, iv in enumerate(insufficentAmpliconIntervals):
                sys.stderr.write("\r" + progress.show(i))
                try:
                    designIntervalOversize = max(
                        [
                            max(x)
                            for x in config["design"]["primer3"][tier]["PRIMER_PRODUCT_SIZE_RANGE"]
                        ]
                    )
                except:
                    print(
                        "WARNING: could not determine maximum amplicon size, default setting applied",
                        file=sys.stderr,
                    )
            ...
            if designedPairs:
                ...
                with tempfile.NamedTemporaryFile(
                    suffix=".fa", prefix="primers_", delete=False, mode="wt"
                ) as fh:
                    for k, v in designedPairs.items():
                        for pairnumber, pair in enumerate(v):
                            print(
                                pair[0].fasta(
                                    "_".join(
                                        [k.name, str(pairnumber), "rev" if k.strand < 0 else "fwd"]
                                    )
                                ),
                                file=fh,
                            )
                            print(
                                pair[1].fasta(
                                    "_".join(
                                        [k.name, str(pairnumber), "fwd" if k.strand < 0 else "rev"]
                                    )
                                ),
                                file=fh,
                            )
                ...
                if len(pairs) != len(specificPrimerPairs):
                    sys.stderr.write(
                        "Removed "
                        + str(len(pairs) - len(specificPrimerPairs))
                            ...
                            if rename:
                                ...
                                for iv in gplist:  # get intervals from file or commandline
                                    if pair[0].targetposition.chrom == iv.chrom:
                                        targetend = (
                                            pair[1].targetposition.offset
                                            + pair[1].targetposition.length
                                        )
                                        assert targetend > pair[0].targetposition.offset
                                        if (
                                            (
                                                pair[0].targetposition.offset
                                                <= iv.chromStart
                                                <= targetend
                                            )
                                            or (
                                            ...
                                            or (iv.chromStart <= targetend <= iv.chromEnd)
                                        ):
                                            matches.append(iv)
                                            # break
                                if len(matches) == 0:
                                    ...
                                    new_name = ",".join(
                                        "{0}_{1}".format(gene, range_string(sorted(exons)))
                                        for (gene, exons) in exons.items()
                                        if random_gene_names_re.match(gene) is None
                                    )
                                    if new_name == "":
                                        new_name = ",".join(
                                            "{0}_{1}".format(gene, range_string(sorted(exons)))
                ...
                if failCount:
                    print(
                        "INFO: {:2d} pairs violated design limits and were blacklisted ({} total)".format(
                            failCount, str(len(blacklist))
                        ),
                        file=sys.stderr,
                    )
                    flash_messages.append(
                        (
                            "{:2d} pairs violated design limits and were blacklisted ({} total)".format(
                                failCount, str(len(blacklist))
                            ),
                            "info",
                        )
                ...
                for k, v in intervalprimers.items():
                    if len(v) == 0:
                        failmsg = "WARNING: Target {} failed on designlimits in tier {}".format(
                            k, tier
                        )
    ...
    print(
        "got primers for {:d} out of {:d} targets".format(
            len(ivpairs) - len(failure), len(ivpairs)
        ),
        file=sys.stderr,
    )
    ...
    for iv, p in sorted(ivpairs.items(), key=lambda x: x[0].name):
        print(
            "{:<20} {:9} {:<10}".format(
                unquote(iv.name), len(p), "FAIL" if len(p) < config["report"]["pairs"] else "OK"
            ),
            file=sys.stderr,
        )
    ...
    if compatible:
        ...
        if rankedPairs:
            ...
            gapInterval = Interval(
                intervals[0].chrom, intervals[0].chromStart, intervals[1].chromEnd
            )
            gapPrimerPair = PrimerPair(
                [rankedPairs[0][2][0], rankedPairs[0][3][1]], name="gap_" + gapInterval.name
            )
        ...
        for iv in sorted(ivpairs.keys()):
            ...
            print(
                ...
                "names",
                [
                    (ivpair.name, ivpair.designrank(), ivpair.original_name)
                    for ivpair in ivpairs[iv]
                ],
                file=sys.stderr,
            )
            if not ivpairs[iv]:
...
def zippyPrimerQuery(
    ...
    if isinstance(targets, tuple):
        intervalforlocus = readTargets(
            targets[1], config["tiling"]
        )  # get intervals from file or commandline
        intervalsforfile = readTargets(
            targets[0], config["tiling"]
        )  # get intervals from file or commandline
        overlappings = []
    ...
    if gap:  # gap PCR primers
        try:
            ...
            print(
                "ERROR: gap-PCR primers can only be designed for a single pair of breakpoint intervals on the same chromosome!",
                file=sys.stderr,
            )
            flash_messages.append(
                (
    ...
    primerTable, resultList, missedIntervals, more_flash_messages = getPrimers(
        ...
        config,
        tiers,
        compatible=True if gap else False,
...
def zippyBatchQuery(
    config, targets, design=True, outfile=None, db=None, predesign=False, tiers=[0]
):
    flash_messages = []
    ...
    for sample, intervals in sorted(sampleVariants.items(), key=lambda x: x[0]):
        print(
            "Getting primers for {} variants in sample {}".format(len(intervals), sample),
            file=sys.stderr,
        )
        # get/design primers
        primerTable, resultList, missedIntervals, more_flash_messages = getPrimers(
            intervals, db, design, config, tiers, rename=shortHumanReadable
        )
        flash_messages.extend(more_flash_messages)
        if missedIntervals:
    ...
    if not outfile:
        ...
        worksheetName = (
            ""
            if os.path.basename(targets[0]).startswith(os.path.basename(outfile))
            else os.path.basename(outfile)
        )
        # output data
        ...
        if primerTests:
            ...
            ws.fillPlates(
                size=config["report"]["platesize"],
                randomize=True,
                includeSamples=False,
                includeControls=True,
            )  # only include controls
            ws.createWorkSheet(
                writtenFiles[-1], primertest=True, worklist=worksheetName, **config["report"]
            )
            # robot csv
...
def updateLocation(primername, location, database, force=False):
    ...
    if not occupied or force:
        if database.storePrimer(primername, location, force):
            print(
                "%s location sucessfully set to %s" % (primername, str(location)), file=sys.stderr
            )
            return ("success", location)
        else:
            print(
                "WARNING: %s location update to %s failed" % (primername, str(location)),
                file=sys.stderr,
            )
        ...
        for line in readfile:
            if not header:
                ...
                try:
                    ...
                    updateList.append(
                        [row["PrimerName"], Location(row["Box"].strip("Bbox"), row["Well"])]
                    )
                except:
                    raise
...
def main():
    ...
    global_group.add_argument(
        "-c",
        dest="config",
        default=zippy_json_path,
        metavar="JSON_FILE",
        help="configuration file [{0}]".format(zippy_json_path),
    )

    global_group.add_argument(
        "--tiers", dest="tiers", default="0,1,2", help="Allowed design tiers (0,1,...,n)"
    )
    ...
    parser_retrieve.add_argument(
        "--outfile", dest="outfile", default="", type=str, help="Output file name"
    )
    parser_retrieve.set_defaults(which="get")

    ## query database for primers by name
    parser_query = subparsers.add_parser(
        "query", help="Query database for primers with specified sub-string in name"
    )
    parser_query.add_argument(
        "subString",
        default="",
        nargs="?",
        metavar="Sub-string within name",
    ...
    parser_batch.add_argument(
        "targets", default=None, metavar="FILE1,FILE2,...", help="SNPpy result table(s) "
    )
    parser_batch.add_argument(
        "--predesign",
        dest="predesign",
    ...
    if options.which == "add":  # read primers and add to database
        ...
        if options.primers.split(".")[-1].startswith("fa"):
            pairs = importPrimerPairs(
                options.primers, config, primer3=False
            )  # import and locate primer pairs
        ...
        if options.amplicons:
            ...
            print(
                "What to dump?. Options are --amplicons, --ordersheet, --locations, --redundancies, --table, --outfile <outfile>, "
        ...
        if options.blacklist:
            print(
                "BLACKLISTED PAIRS: {}".format(",".join(db.blacklist(options.blacklist))),
                file=sys.stderr,
            )
        ...
        zippyPrimerQuery(
            ...
            db,
            options.store,
            list(map(int, options.tiers.split(","))),
            options.gap,
        )
