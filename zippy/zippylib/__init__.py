...
def githash(prefix=None):
    ...
    for i, line in enumerate(
        subprocess.check_output(["git", "show-ref", "--head", "--abbrev=7"], cwd=APP_ROOT).split(
            "\n"
        )
    ):
        f = line.split()
...
def flatten(container):
    ...
    if (
        ...
        and type(container) is not tuple
        and type(container) is not PrimerPair
    ):
        yield container
    else:
        for i in container:
...
class Progressbar(object):
    ...
    def show(self, i):
        ...
        eta = (
            str(int((self.total - i) * float(time.time() - self.start) / float(i)))
            if i and i / float(self.total) > 0.02
            else "?"
        )
        return ("{name:} [{progress:<" + str(self.maxlen) + "}] {done:} (ETA {eta:>2}s)").format(
            name=self.name,
            progress=self.char
            * (int(self.maxlen * i / float(self.total)) if self.total != 0 else self.maxlen),
